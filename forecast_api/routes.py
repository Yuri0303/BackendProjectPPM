...
from models.daily_user_request import DailyIpRequest
...
class ForecastView(MethodView):
    def get(self):
        ...
        if user_id:
            if User.query.filter_by(id=user_id).first().is_admin:
                return jsonify({'error': 'Admin can\'t save queries. Please log in with a Premium user account'}), 403
            ...
            if record:
                if record.count >= 10:
                    return jsonify({'error': 'Daily request limit reached (10)'})
                else:
                    record.count += 1
            else:
    ...
    def post(self):
        ...
        if not user.is_admin:
            return jsonify({'error': 'Only Admin can add forecasts'}), 403
...
def saved_queries():
    ...
    if User.query.filter_by(id=user_id).first().is_admin:
        return jsonify({'error': 'Admin don\'t have saved queries. Please log in with a Premium User account'}), 403
...
def add_location():
    ...
    if not user.is_admin:
        return jsonify({'error': 'Only admin can add locations'}), 403
